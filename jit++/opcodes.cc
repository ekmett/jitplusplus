...
namespace jitpp { 
    ...
    template <typename T> static inline int64_t sal(interpreter & i, int64_t x, int count) { 
        static const int size = sizeof(T)*8;
        static const int mask = size == 64 ? 0x3f : 0x1f;
        count &= mask;
        if (!count) return x;
        return i.handle_rflags(flags::typed<T>::sal,x << count,x,count);
    }
    template <typename T> static inline int64_t shr(interpreter & i, int64_t x, int count) { 
        static const int size = sizeof(T)*8;
        static const int mask = size == 64 ? 0x3f : 0x1f;
        count &= mask;
        if (!count) return x;
        return i.handle_rflags(flags::typed<T>::shr,x >> count,x,count);
    }

    template <typename T> static inline int64_t sbb(interpreter & i, int64_t x, int64_t y) {
            if (i.cf()) return i.handle_rflags(flags::typed<T>::sbb,x-y-1,x,y);
            else return i.handle_rflags(flags::sub,x-y,x,y);
        }
    
    template <typename T> static inline int64_t adc(interpreter & i, int64_t x, int64_t y) {
            if (i.cf()) return i.handle_rflags(flags::adc,x+y+1,x,y);
            else return i.handle_rflags(flags::add,x+y,x,y);
        }
    
    template <typename T> static inline int64_t add(interpreter & i, int64_t x, int64_t y) {
            return i.handle_rflags(flags::add,x+y,x,y);
        }
    
    template <typename T> static inline int64_t sub(interpreter & i, int64_t x, int64_t y) {
            return i.handle_rflags(flags::sub,x-y,x,y);
        }
    
    template <typename T> static inline int64_t and_(interpreter & i, int64_t x, int64_t y) {
            return i.handle_rflags(flags::logic,x&y);
        }
    
    template <typename T> static inline int64_t or_(interpreter & i, int64_t x, int64_t y) {
            return i.handle_rflags(flags::logic,x|y);
        }
    
    template <typename T> static inline int64_t xor_(interpreter & i, int64_t x, int64_t y) {
            return i.handle_rflags(flags::logic,x^y);
        }
...
#define SBB(T,x,y) sbb<T>(i,(x),(y))
#define CMP(T,x,y) add<T>(i,(x),(y))
#define XOR(T,x,y) xor_<T>(i,(x),(y))
#define AND(T,x,y) and_<T>(i,(x),(y))
#define OR(T,x,y)  or_<T>(i,(x),(y))
#define ADC(T,x,y) adc<T>(i,(x),(y))
#define INC(T,x)   inc<T>(i,(x))
#define DEC(T,x)   dec<T>(i,(x))
#define NEG(T,x)   neg<T>(i,(x))
#define SAL(T,x,y) sal<T>(i,(x),(y))
#define SHR(T,x,y) shr<T>(i,(x),(y))
#define SAR(T,x,y) sar<T>(i,(x),(y))
#define ROL(T,x,y) rol<T>(i,(x),(y))
#define ROR(T,x,y) ror<T>(i,(x),(y))
#define RCL(T,x,y) rcl<T>(i,(x),(y))
#define RCR(T,x,y) rcr<T>(i,(x),(y))
...
#define I i.op.imm
    ...
    template <typename os> static void interpret_opcode(interpreter & i) { 
        ...
        case 0x04: reg<b>(i,0,ADD(b,reg<b>(i,0),I)); return; // ADD AL, Ib
        case 0x05: reg<v>(i,0,ADD(v,reg<v>(i,0),I)); return; // ADD rAX, Iz
        ...
        case 0x0c: reg<b>(i,0,OR(b,reg<b>(i,0),I)); return; // OR AL, Ib
        case 0x0d: reg<v>(i,0,OR(v,reg<v>(i,0),I)); return; // OR rAX, Iz
        ...
        case 0x14: reg<b>(i,0,ADC(b,reg<b>(i,0),I)); return; // ADC AL, Ib
        case 0x15: reg<v>(i,0,ADC(v,reg<v>(i,0),I)); return; // ADC rAX, Iz
        ...
        case 0x1c: reg<b>(i,0,SBB(b,reg<b>(i,0),I)); return; // SBB AL, Ib
        case 0x1d: reg<v>(i,0,SBB(v,reg<v>(i,0),I)); return; // SBB rAX, Iz
        ...
        case 0x24: reg<b>(i,0,AND(b,reg<b>(i,0),I)); return; // AND AL, Ib
        case 0x25: reg<v>(i,0,AND(v,reg<v>(i,0),I)); return; // AND rAX, Iz
        ...
        case 0x2c: reg<b>(i,0,SUB(b,reg<b>(i,0),I)); return; // SUB AL, Ib
        case 0x2d: reg<v>(i,0,SUB(v,reg<v>(i,0),I)); return; // SUB rAX, Iz
        ...
        case 0x34: reg<b>(i,0,XOR(b,reg<b>(i,0),I)); return; // XOR AL, Ib
        case 0x35: reg<v>(i,0,XOR(v,reg<v>(i,0),I)); return; // XOR rAX, Iz
        ...
        case 0x3c: SUB(b,reg<b>(i,0),I); return; // CMP AL, Ib
        case 0x3d: SUB(v,reg<v>(i,0),I); return; // CMP rAX, Iz
        ...
        case 0x54: case 0x55: case 0x56: case 0x57: // PUSH rXX
            PUSH(v,reg<v>(i,i.op.rex_b(i.op.code & 7))); return;
        case 0x58: case 0x59: case 0x5a: case 0x5b: 
        case 0x5c: case 0x5d: case 0x5e: case 0x5f: // POP rXX
            reg<v>(i,i.op.rex_b(i.op.code & 7),POP(v)); return;
        ...
        case 0x7c: case 0x7d: case 0x7e: case 0x7f: // JCC Ib
            if (test_cc(i,i.op.code & 0xf)) 
                RIP += I;
            return;
        case 0x80: // group #1 Eb, Ib
            switch (i.op.reg) { 
            case 0: E<b>(i,ADD(b,E<b>(i),I)); break;
            case 1: E<b>(i,OR(b,E<b>(i),I)); break;
            case 2: E<b>(i,ADC(b,E<b>(i),I)); break;
            case 3: E<b>(i,SBB(b,E<b>(i),I)); break;
            case 4: E<b>(i,AND(b,E<b>(i),I)); break;
            case 5: E<b>(i,SUB(b,E<b>(i),I)); break;
            case 6: E<b>(i,XOR(b,E<b>(i),I)); break;
            case 7: SUB(b,E<b>(i),I); break;
            default: logic_error(); 
            }
            return;
        // case 0x81 below
        ...
        case 0x83: // group #1 Ev, Ib
            switch (i.op.reg) { 
            case 0: E<v>(i,ADD(v,E<v>(i),I)); break;
            case 1: E<v>(i,OR(v,E<v>(i),I)); break;
            case 2: E<v>(i,ADC(v,E<v>(i),I)); break;
            case 3: E<v>(i,SBB(v,E<v>(i),I)); break;
            case 4: E<v>(i,AND(v,E<v>(i),I)); break;
            case 5: E<v>(i,SUB(v,E<v>(i),I)); break;
            case 6: E<v>(i,XOR(v,E<v>(i),I)); break;
            case 7: SUB(v,E<v>(i),I); break;
            default: logic_error(); 
            }
            return;
        ...
        case 0xb8: case 0xb9: case 0xba: case 0xbb: case 0xbc: case 0xbd: case 0xbe: case 0xbf: 
            // MOV RXX, Iq
            reg<v>(i,i.op.rex_b(i.op.code & 7),I);
            return;
        case 0xc0: 
            switch (i.op.reg) {
            case 0: unsupported(); // ROL Eb,1 
            case 1: unsupported(); // ROR Eb,1
            case 2: unsupported(); // RCL Eb,1
            case 3: unsupported(); // RCR Eb,1
            case 4: E<b>(i,SAL(b,E<b>(i),1)); return; // SHL Eb,1
            case 5: E<b>(i,SHR(b,E<b>(i),1)); return; // SHR Eb,1
            case 6: E<b>(i,SAL(b,E<b>(i),1)); return; // SAL Eb,1 = SHL Eb,1
            case 7: unsupported(); // SAR Eb,1
            }
            logic_error();
        case 0xc1: 
            switch (i.op.reg) {
        ...
        case 0xe8: // CALL Jz
            ...
            RIP += I;
            return;
        case 0xe9: RIP += I; return; // JMP Jz
        case 0xea: illegal(); // JMP Ap
        case 0xeb: RIP += I; return; // JMP Jb
        ...
        case 0xff: // opcode group 5
            ...
            case 2: // CALL Ev
                ...
                return;
            case 3: unsupported();
            case 4: RIP = E<v>(i); return; // JMP Ev
            case 5: unsupported();
        ...
        case 0x1b0: // CMPXCHG Eb,Gb
            {
                int8_t value = E<b>(i);
                if (i.zf(value == reg<b>(i,0))) E<b>(i,G<b>(i));
                else reg<b>(i,0,value);
                return;
            }
        case 0x1b1: // CMPXCHG Ev,Gv
            {
                int8_t value = E<v>(i);
                if (i.zf(value == reg<v>(i,0))) E<v>(i,G<v>(i));
                else reg<v>(i,0,value);
                return;
            }
