...
namespace jitpp {
    const uint8_t opcode::encoding_lut[512] = {
	// xx
    ...
    int64_t opcode::parse(int64_t i) {
	parts = prefix = seg_prefix = sse_prefix = 0;
    refetch:
        code = fetch<uint8_t>(i);
	VLOG(1) << "opcode = " << std::hex << (int)code;
	...
	parts |= encoding_flags & (encoding_has_modrm | encoding_extra_byte | encoding_drex_byte);
		...
		VLOG(1) << "extra = " << std::hex << (int)extra;
	    }
	    // parse mod r/m
