...
AC_DEFUN([AC_CXX_STL_NAMESPACE],
  [AC_CACHE_CHECK(
   ...
   if test "$ac_cv_cxx_stl_namespace" = none; then
      AC_DEFINE(STL_NAMESPACE,,[the namespace where STL code like vector<> is defined])
   fi
   if test "$ac_cv_cxx_stl_namespace" = std; then
      AC_DEFINE(HAVE_STL_NAMESPACE,1,[we have a non-empty STL_NAMESPACE])
      AC_DEFINE(STL_NAMESPACE,std,[the namespace where STL code like vector<> is defined])
   fi
])
